...
export const ProductSearchBar: React.FC<ProductSearchBarProps> = ({ className = "", categories, brands, searchFilter,  categoryFilter, brandFilter}) => {
    ...
    const handleBrandSelectChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    return(
        <div className={`${className} searchContainer`}>
            <label>
                ...
                <input placeholder="Buscar Producto" type='search'value={searchValue} onChange={handleSearchInputChange}/>
            </label>
            <select defaultValue="" className="searchSelect" value={brand} onChange={handleBrandSelectChange}>
                <option hidden value="">Línea</option>
                {brands.map((brand: IBrand) => (
                    <option value={brand.id}>{brand.name}</option>
                ))}
            </select>
            <select defaultValue="" className="searchSelect" value={category} onChange={handleCategorySelectChange}>
